...
/*
 * Channel States - Variables and Functions for off chain transactions and signing
 */
...
contract('TokenChannels', accounts => {
  ...
  const joinChannel = async () => {
    ...
    // balance of channel after bob joins should be equal to sum of alice and bobs deposit amounts
  ...
  describe('channel without challenge period', () => {
    describe('open a channel', () => {
      ...
      it("should not open a channel with himself", async () => {
        const amount = aliceDeposit;
        const open = channel.open(tokenAddress, alice, amount, challengePeriod, {
          from: alice
        });
        await expectRevert(open, "You cannot create a channel with yourself.");
      });

      it("should not open a channel without tokens", async () => {
        const amount = '0';
        const open = channel.open(tokenAddress, bob, amount, challengePeriod, { from: alice });
        await expectRevert(open, "You cannot create a payment channel without tokens.");
      });

      it("should not open a channel without token transferFrom approval", async () => {
      ...
      it("should not open a channel with insufficient tokens", async () => {
    ...
    describe('join to a channel', () => {
      ...
      it("should not join to a invalid channel", async () => {
      ...
      it("should not join if the user is not the counter party", async () => {
        const { channelId } = channelStates.pop();
        ...
        await expectRevert(join, "The channel creator did not specify you as the counter party. Check with them off-chain.");
      });
      ...
      it("should not join to a closed", async () => {
        await joinChannel();
    ...
    describe('close a channel', () => {
      ...
      it("the channel should not be closed by a third-party", async () => {
        const state = getLastState();
        await expectRevert(closeChannel(state, carl), 'You are not a participant in this channel.');
      });

      describe("the channel should not be closed using wrong signatures", async () => {
  ...
  describe('channel with challenge period', () => {
    ...
    describe('join to a channel', () => {
      ...
      it("should not join to a channel with on challenge status", async () => {
        await joinChannel();
        await offChainTransfers();
        const lastState = getLastState();
        await closeChannel(lastState, alice);
    ...
    describe('redeem funds', () => {
      ...
      it("the funds should not be redeemed during the challenge period", async () => {
